...
public class GetInvitesUseCaseTests(DvizhWebApplicationFactory factory) : IClassFixture<DvizhWebApplicationFactory>
{
    ...
    [Fact]
    public async Task Execute_WithExpiresAtFilter_ExcludesExpiredInvites()
    {
    ...
    [Fact]
    public async Task Execute_WithExpiresAtFilter_ExcludesActiveInvites()
    {
        var ct = TestContext.Current.CancellationToken;
        await using var db = new DatabaseScope(_factory);
        var active = await db.SeedInvite(i =>
    ...
    public async Task Execute_NoFilter_IncludesAll()
    {
        ...
        var input = new GetInvitesInput(new SieveModel { PageSize = 100 });
        var result = await useCase.Execute(input, ct);
    ...
    public async Task Execute_Pagination_ReturnsTotalCount()
    {
        ...
        var input = new GetInvitesInput(new SieveModel { Page = 1, PageSize = 1 });
        var result = await useCase.Execute(input, ct);
