...
export function Pagination({ page, pageSize, totalPages, totalCount, onPageChange, onPageSizeChange }: PaginationProps) {
  ...
  return (
    <div className="flex flex-wrap items-center justify-between gap-3 pt-4">
      <span className="text-xs text-gray-400">{from}–{to} of {totalCount}</span>
      ...
      <div className="flex items-center gap-3">
        <div className="flex items-center gap-1">
          ...
          <span className="text-xs text-gray-400 ml-0.5">per page</span>
        </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center gap-0.5">
            ...
            {pages.map((p, i) =>
              p === '...'
                ? <span key={`e-${i}`} className="w-8 h-8 flex items-center justify-center text-sm text-gray-300">…</span>
                : <button
                    key={p}
                    onClick={() => onPageChange(p)}
                    className={`${btn} ${p === page ? 'bg-indigo-500 text-white font-medium' : 'text-gray-500 hover:bg-gray-100'}`}
                  >
                    {p}
                  </button>
            )}
